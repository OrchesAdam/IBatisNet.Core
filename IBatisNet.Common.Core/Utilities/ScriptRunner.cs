...
namespace IBatisNet.Common.Utilities
{
    ...
    public class ScriptRunner
    {
        ...
        /// <returns></returns>
        private List<string> GetSqlStatements(IDataSource dataSource, string script, bool doParse)
        {
            ...
            if (doParse)
            {
                switch (dataSource.DbProvider.Name)
                {
                    ...
                    case "MySql":
                        ...
                        break;
                    case "OleDb1.1":
                        if (dataSource.ConnectionString.IndexOf("Microsoft.Jet.OLEDB", StringComparison.Ordinal) > 0)
                        {
                            // Access
                            sqlStatements = ParseScript(script);
                        }
        ...
        {
            // Get script file
            var fi = new FileInfo(sqlScriptPath);
            var script = fi.OpenText().ReadToEnd();
            var sqlStatements = GetSqlStatements(dataSource, script, doParse);
            try
            {
        ...
        /// <summary>
        /// Run an sql script asynchronous
        /// </summary>
        /// <param name="dataSource">The dataSouce that will be used to run the script.</param>
        /// <param name="sqlScriptPath">a path to an sql script file.</param>
        public async Task RunScriptAsync(IDataSource dataSource, string sqlScriptPath)
        {
            await RunScriptAsync(dataSource, sqlScriptPath, true);
        }
        ...
        /// <param name="sqlScriptPath">a path to an sql script file.</param>
        /// <param name="doParse">parse out the statements in the sql script file.</param>
        public async Task RunScriptAsync(IDataSource dataSource, string sqlScriptPath, bool doParse)
        {
            // Get script file
            var fi = new FileInfo(sqlScriptPath);
        ...
        private void ExecuteStatements(IDataSource dataSource, IEnumerable<string> sqlStatements)
        {
            var connection = dataSource.DbProvider.CreateConnection();
        ...
        /// <param name="dataSource">The dataSouce that will be used.</param>
        /// <param name="sqlStatements">An ArrayList of sql statements to execute.</param>
        private async Task ExecuteStatementsAsync(IDataSource dataSource, IEnumerable<string> sqlStatements)
        {
            var connection = dataSource.DbProvider.CreateConnection();
            connection.ConnectionString = dataSource.ConnectionString;
            connection.Open();
        ...
        private List<string> ParseScript(string script)
        {
            ...
            var parser = new StringTokenizer(script, ";");
